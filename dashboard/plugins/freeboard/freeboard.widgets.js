...
(function() {
  ...
  var SPARKLINE_COLORS = ["#FF9900", "#FFFFFF", "#B3B4B4",
    "#6B6B6B", "#28DE28", "#13F7F9", "#E6EE18", "#C41204",
    "#CA3CB8", "#0B1CFB"
  ];
  ...
  function easeTransitionText(newValue, textElement, duration) {
    ...
    if ($.isNumeric(newValue) && $.isNumeric(currentValue)) {
      ...
      jQuery({
        ...
        duration: duration,
        step: function() {
          $(textElement).text(this.transitionValue.toFixed(
            this.precisionValue));
        },
        done: function() {
  ...
  function addValueToSparkline(element, value, legend) {
    ...
    _.each(values, function(valueArray, valueIndex) {
      $(element).sparkline(valueArray, {
        ...
        width: "100%",
        fillColor: false,
        lineColor: SPARKLINE_COLORS[valueIndex %
          SPARKLINE_COLORS.length],
        lineWidth: 2,
        spotRadius: 3,
        spotColor: false,
        minSpotColor: "#78AB49",
        ...
        tooltipFormat: (legend && legend[valueIndex]) ?
  ...
  freeboard.addStyle('.widget-big-text', valueStyle +
    "font-size:75px;");
  ...
  var textWidget = function(settings) {
    ...
    var titleElement = $(
      '<h2 class="section-title tw-title tw-td"></h2>');
    var valueElement = $('<div class="tw-value"></div>');
    var unitsElement = $('<div class="tw-unit"></div>');
    var sparklineElement = $(
      '<div class="tw-sparkline tw-td"></div>');

    function updateValueSizing() {
      if (!_.isUndefined(currentSettings.units) && currentSettings
        .units != "") // If we're displaying our units
      {
        valueElement.css("max-width", (displayElement
        .innerWidth() - unitsElement.outerWidth(true)) + "px");
      } else {
    ...
    this.render = function(element) {
      ...
      $(displayElement)
        ...
        .append($('<div class="tw-tr"></div>').append($(
          ...
          .append(valueElement).append(unitsElement)))
        .append($('<div class="tw-tr"></div>').append(
          sparklineElement));
    ...
    this.onSettingsChanged = function(newSettings) {
      ...
      var shouldDisplayTitle = (!_.isUndefined(newSettings
        .title) && newSettings.title != "");
      var shouldDisplayUnits = (!_.isUndefined(newSettings
      ...
      if (shouldDisplayTitle) {
        titleElement.html((_.isUndefined(newSettings.title) ?
          "" : newSettings.title));
      ...
      if (shouldDisplayUnits) {
        unitsElement.html((_.isUndefined(newSettings.units) ?
          "" : newSettings.units));
    ...
    this.onCalculatedValueChanged = function(settingName,
      newValue) {
    ...
    this.getHeight = function() {
      if (currentSettings.size == "big" || currentSettings
        .sparkline) {
        return 2;
      } else {
        return 1;
  ...
  freeboard.addStyle('.gauge-widget',
  ...
  var gaugeWidget = function(settings) {
    ...
    var gaugeElement = $('<div class="gauge-widget" id="' +
      thisGaugeID + '"></div>');
    ...
    function createGauge() {
      ...
      gaugeObject = new JustGage({
        id: thisGaugeID,
        value: (_.isUndefined(currentSettings.min_value) ? 0 :
          currentSettings.min_value),
        min: (_.isUndefined(currentSettings.min_value) ? 0 :
          currentSettings.min_value),
        max: (_.isUndefined(currentSettings.max_value) ? 0 :
    ...
    this.render = function(element) {
      rendered = true;
      $(element).append(titleElement).append($(
  ...
  var sparklineWidget = function(settings) {
    ...
    this.onSettingsChanged = function(newSettings) {
      ...
      if (newSettings.include_legend) {
        addSparklineLegend(sparklineLegend, newSettings.legend
          .split(","));
      }
    }
    ...
    this.getHeight = function() {
      var legendHeight = 0;
      if (currentSettings.include_legend && currentSettings
        .legend) {
        var legendLength = currentSettings.legend.split(",")
          .length;
        if (legendLength > 4) {
          legendHeight = Math.floor((legendLength - 1) / 4) *
            0.5;
  ...
  var pointerWidget = function(settings) {
    ...
    this.render = function(element) {
      ...
      triangle = paper.path(polygonPath([width / 2, (height /
        2) - radius + strokeWidth, 15, 20, -30, 0
      ]));
      triangle.attr("stroke-width", 0);
      triangle.attr("fill", "#fff");

      $(element).append($('<div class="pointer-value"></div>')
        .append(valueDiv).append(unitsDiv));
    }
  ...
  var pictureWidget = function(settings) {
    ...
    function updateImage() {
      if (widgetElement && imageURL) {
        var cacheBreakerURL = imageURL + (imageURL.indexOf("?") ==
          -1 ? "?" : "&") + Date.now();
    ...
    this.onSettingsChanged = function(newSettings) {
      ...
      if (newSettings.refresh && newSettings.refresh > 0) {
        timer = setInterval(updateImage, Number(newSettings
          .refresh) * 1000);
      }
    }
  ...
  var indicatorWidget = function(settings) {
    ...
    function updateState() {
      ...
      if (isOn) {
        stateElement.text((_.isUndefined(onText) ? (_.isUndefined(
            currentSettings.on_text) ? "" : currentSettings
          .on_text) : onText));
      } else {
        stateElement.text((_.isUndefined(offText) ? (_
          .isUndefined(currentSettings.off_text) ? "" :
          currentSettings.off_text) : offText));
      }
    }

    this.render = function(element) {
      $(element).append(titleElement).append(indicatorElement)
        .append(stateElement);
    }
  ...
  var googleMapWidget = function(settings) {
    ...
    function updatePosition() {
      if (map && marker && currentPosition.lat && currentPosition
        .lon) {
        var newLatLon = new google.maps.LatLng(currentPosition
          .lat, currentPosition.lon);
    ...
    this.render = function(element) {
      function initializeMap() {
        ...
        google.maps.event.addDomListener(element, 'mouseenter',
          function(e) {
            e.cancelBubble = true;
            if (!map.hover) {
              map.hover = true;
        ...
        google.maps.event.addDomListener(element, 'mouseleave',
          function(e) {
            if (map.hover) {
              map.setOptions({
                zoomControl: false
